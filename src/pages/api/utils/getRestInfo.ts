...
const getRestDeInfo = async () => {
  ...
  const [prevMonthRestDeInfoResponse, nowMonthRestDeInfoResponse] =
    await Promise.all([
      fetch(
  ...
  const prevMonthRestInfo =
    (await prevMonthRestDeInfoResponse.json()) as RestDeInfoResponse;
  const nowMonthRestInfo =
    (await nowMonthRestDeInfoResponse.json()) as RestDeInfoResponse;

  const prevMonthRestInfoArray = Array.isArray(
    prevMonthRestInfo.response.body.items.item,
  )
    ? prevMonthRestInfo.response.body.items.item
    : [prevMonthRestInfo.response.body.items.item];
  const nowMonthRestInfoArray = Array.isArray(
  ...
  const restArray = [
    ...prevMonthRestInfoArray,
    ...nowMonthRestInfoArray,
  ].filter((v) => !!v);
